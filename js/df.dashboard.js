 ...
 * Flashes stuff
 * @param element
 * @param bad If true, turns red instead of green
 * @private
 */
var _flasher = function(element, bad) {
	var _toColor = 'linear-gradient(#468847, #79BA7A)';

	if (jQuery(element) && jQuery(element).length) {
		if (bad) {
			_toColor = 'linear-gradient(red, darkred)';
//			_toColor = 'linear-gradient(#EE5F5B, #BD362F)';
		}

		jQuery(element).animateGradient(_toColor, 500, function() {
			jQuery(element).animateGradient('linear-gradient(#f5f4f4, #c7c7c7)', 500, function() {
				jQuery(element).animateGradient(_toColor, 500, function() {
					jQuery(element).animateGradient('linear-gradient(#f5f4f4, #c7c7c7)', 500, function() {
						jQuery(element).animateGradient(_toColor, 500, function() {
							jQuery(element).animateGradient('linear-gradient(#f5f4f4, #c7c7c7)', 500);
						});
					});
				});
			});
...
var _checkProgress = function() {
	(function($) {
		...
		$(".instance-heading-status .fa-spinner").each(function(index, item) {
			...
			var _div = $(this).closest('.accordion-toggle').attr('href');
			...
			if (_div && _div.length) {
				...
				if (3 == _parts.length) {
					...
					$.post('/web/index', $_form.serialize(), function(data) {
...
//								_flasher($(item).closest('.accordion-group').find('.instance-heading'), true);
							...
							_closeAlert('#alert-status-change', 30000, true);
						}
						else {
...
//								_flasher($(item).closest('.accordion-group').find('.instance-heading'));
	...
	if (!_checking && jQuery("div.dsp-list .panel-group .panel-heading span.instance-heading-status i.fa.fa-spinner").length) {
		_checking = true;
		window.setTimeout(_checkProgress, 10000);
	}
};
...
jQuery(function($) {
	...
	window.setTimeout(_checkProgress, 10000);
	_closeAlert('.system-alert', 30000);
});
